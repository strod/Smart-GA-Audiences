WITH
    ...
    *
    ...
    FROM
        `{table}` g, UNNEST(g.hits) h
    WHERE
        _TABLE_SUFFIX BETWEEN '{conversion_window_start_date}' AND '{conversion_window_end_date}' 
        AND clientId IS NOT NULL
        AND  h.eventInfo.eventCategory = 'ecommerce' 
        AND  h.eventInfo.eventAction = 'adesao' 
        AND UPPER(h.eventInfo.eventLabel) LIKE '%CONTROLE%'
    GROUP BY 1)
    WHERE 
    y_conversions > 0
    ),
